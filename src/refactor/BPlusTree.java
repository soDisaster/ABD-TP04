...
public class BPlusTree {
	 ...
	 * @return la valeur associée à cette clé si elle est présente dans le B+-Tree, null sinon
	 */
	...
	private Object search(Node node, int key) {
		...
		/* Parcours des clés à la recherche de l'indice de la clé passée en paramètre, si elle est présente */
	...
	/**
	 * Recherche d'une clé dans le B+-Tree et retourne le Node contenant cette clé
	 * 
	 * @param key
	 * 			Clé à chercher dans le B+-Tree
	 * @return le Node qui contient cette clé si elle est présente dans le B+-Tree, null sinon
	 */
	...
	/**
	 * Split le noeud passé en paramètre en deux noeuds qui contiennent respectivement degré/2 - 1 et degré/2 éléments
	 * Ajoute sa clé centrale aux clés du noeud parent passé en paramètre
	 * 
	 * Cette méthode est appelée uniquement si le noeud est plein
	 * Le noeud passé en paramètre doit être le i-ème fils du noeud parentNode
	 * 
	 * @param parentNode
	 * 			Noeud parent
	 * @param i
	...
	private void splitChildNode(Node parentNode, int i, Node node) {
		...
		/* Copie les degré/2 derniers éléments du noeud passé en paramètre dans newNode */
		for (int j = 0; j < (this.degree / 2); j++) {
			newNode.keys[j] = node.keys[j + (this.degree / 2) - 1];
			newNode.values[j] = node.values[j + (this.degree / 2) - 1];
		}
		
		/* Si le nouveau noeud est un noeud interne, copie les degré/2 + 1 pointeurs de childNodes dans newNode */
		if (!newNode.isLeafNode) {
			...
			/* Si le nouveau noeud est une feuille, on l'ajoute à la liste des feuilles */
	...
	void insertIntoNonFullNode(Node node, int key, Object object) {
		...
		if (node.isLeafNode) {
			...
			/* Parcours des clés de node jusqu'à trouver le pointeur sur le noeud qui fait office */
			/* de racine pour le sous-arbre du nouvel élément */
			while (i >= 0 && key < node.keys[i]) {
				i--;
			}
			i++;
	...
	private class Node {
		...
		/**
		 * Vérifie si le noeud est plein, c'est à dire qu'il contient déjà le maximum de clés
		 * 
